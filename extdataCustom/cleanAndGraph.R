...
cleanAndGraph <- function(assocFile, infoFile, dataOutputPrefix, macFilter, mafFilter, traitType){
  library(qqman)
  library(data.table)
  library(readr)
  library(plyr)
  ...
  autosomalAssocResults <- fread(assocFile, header=T)
  
  # merge info fields
  infoFields <- fread(infoFile , header = T)
  ...
  bmp(filename=paste(dataOutputPrefix, "qqplot_commonSNPs_cleaned.bmp", sep="_"), width=800, height=800, bg="white", type="cairo")
  par(mar=c(5,6,4,1) + .1)
  ...
  expected <- c(1:length(observed)) 
  lexp <- -(log10(expected / (length(expected)+1)))
  bmp(filename=paste(dataOutputPrefix, "qqplot_rareSNPs_cleaned.bmp", sep="_"), width=800, height=800, bg="white", type="cairo")
  par(mar=c(5,6,4,1) + .1)
  plot(c(0,10), c(0,10), col="red", lwd=3, type="l", xlab="Expected (-logP)", ylab="Observed (-logP)", xlim=c(0,10), ylim=c(0,10), las=1, xaxs="i", yaxs="i", bty="l", cex.axis=2.0, cex.lab=2.0)
  points(lexp, lobs, pch=23, cex=.4, bg="black") 
  #inflation factor lambda
  ...
  bmp(filename=paste(dataOutputPrefix, "manhattan_commonSNPs_cleaned.bmp", sep="_"), width=800, height=600, bg="white", type="cairo")
  par(font.axis = 2)
  ...
  bmp(filename=paste(dataOutputPrefix, "manhattan_rareSNPs_cleaned.bmp", sep="_"), width=800, height=600, bg="white", type="cairo")
  par(font.axis = 2)
  substringPath <- strsplit(dataOutputPrefix, split = "/")
  prefix <- substringPath[[1]][length(substringPath[[1]])]
